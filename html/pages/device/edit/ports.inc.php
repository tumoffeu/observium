...
<table class="table table-striped table-condensed">
  <thead>
    ...
    <tr>
      ...
      <td><button class="btn btn-xs btn-primary" type="submit" value="Toggle"   id="threshold-toggle"  title="Toggle threshold for all ports">Toggle</button>
          <button class="btn btn-xs btn-default" type="submit" value="Select"   id="threshold-select"  title="Disable threshold on all ports">All</button></td>
      <td> Warning</td>
      <td></td>
      <td> Critical</td>
      <td></td>
...
foreach (dbFetchRows("SELECT * FROM `ports` WHERE `deleted` = '0' AND `device_id` = ? ORDER BY `ifIndex` ", array($device['device_id'])) as $port)
{
  ...
  echo('<td>');
  $item = array('id'          => 'threshold_'.$port['port_id'],
                'size'        => 'mini',
                'on-text'     => 'Yes',
                'on-color'    => 'danger',
                'off-text'    => 'No',
...
######################################## Threshold warning #############################################
  ...
  $thresh_warn_custom_bool = isset($port['thresh_warn_custom']);
  $thresh_warn = $thresh_warn_custom_bool ? $port['thresh_warn_custom'] : $port['thresh_warn'];
  $item = array('id'          => 'thresh_warn_custom_'.$port['port_id'],
                //'name'        => 'Group name',
                'placeholder' => $port['thresh_warn'],
                'disabled'    => !$thresh_warn_custom_bool,
                'width'       => '55px',
                'readonly'    => $readonly,
                'ajax'        => TRUE,
                'ajax_vars'   => array('field' => 'thresh_warn'),
                'value'       => $thresh_warn);
                #'value'       => formatRates($thresh_warn, 4, 4));
  echo(generate_form_element($item, 'text'));
  echo('</td>');
  
  echo '<td>';
  // Custom port vars toggle switch
  $item = array('id'       => 'thresh_warn_custom_bool_'.$port['port_id'],
                        'size'     => 'large',
                        ...
                        'onchange-id' => "thresh_warn_custom_".$port['port_id'], // target id for onchange, set attrib: data-onchange-id
                        'value' => (bool)$thresh_warn_custom_bool);
...
######################################## Threshold critical ###########################################
  echo '<td class="text-nowrap">';
  $thresh_crit_custom_bool = isset($port['thresh_crit_custom']);
  $thresh_crit = $thresh_crit_custom_bool ? $port['thresh_crit_custom'] : $port['thresh_crit'];
  $item = array('id'          => 'thresh_crit_custom_'.$port['port_id'],
                //'name'        => 'Group name',
                'placeholder' => $port['thresh_crit'],
                'disabled'    => !$thresh_crit_custom_bool,
                'width'       => '55px',
                'readonly'    => $readonly,
                'ajax'        => TRUE,
                'ajax_vars'   => array('field' => 'thresh_crit'),
                'value'       => $thresh_crit);
                #'value'       => formatRates($thresh_crit, 4, 4));
  echo(generate_form_element($item, 'text'));
  echo('</td>');

  echo '<td>';
  // Custom port vars toggle switch
  $item = array('id'       => 'thresh_crit_custom_bool_'.$port['port_id'],
                        'size'     => 'large',
                        'view'     => $locked ? 'lock' : 'square', // note this is data-tt-type, but 'type' key reserved for element type
                        'onchange' => "toggleAttrib('disabled', obj.attr('data-onchange-id'));",
                        'onchange-id' => "thresh_crit_custom_".$port['port_id'], // target id for onchange, set attrib: data-onchange-id
                        'value' => (bool)$thresh_crit_custom_bool);
          echo(generate_form_element($item, 'toggle'));
  echo '</span>';
  echo('</td>');
#######################################################################################################
...
<script type="text/javascript">
  ...
  $('#threshold-toggle').click(function(event) {
    // invert selection on all threshold buttons
    event.preventDefault();
    $('[id^="threshold_"]').each(function() {
      var id = $(this).attr('id');
      // get the interface number from the object name
      var port_id = id.split('_')[1];
      // find its corresponding checkbox and toggle it
      $('[id="threshold_' + port_id + '"]').bootstrapSwitch('toggleState');
    });
  });
  ...
  $('#threshold-select').click(function(event) {
    // select all threshold buttons
    event.preventDefault();
    $('[id^="threshold_"]').bootstrapSwitch('state', true);
  });
